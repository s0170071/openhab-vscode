...
- Item hover now shows where the item is defined (`.items`) and where it's referenced in Rules (`.rules`), Sitemaps (`.sitemap`) and Scripts (`.js`), each as a clickable, jump-to-location list. Each category and the maximum number of shown locations is configurable (`openhab.hover.*` settings).
